...
$comandos = @(
    @{
        Descricao = "FASE 5: Verificação de Sintaxe (cargo check)"
        Comando = "D:\app\dev\rust\cargo\bin\cargo.exe check --workspace"
        LogFile = "D:\proj\mini\project-mini\logs\fase5-cargo-check.log"
    },
    @{
        Descricao = "FASE 5: Testes Unitários (cargo test)"
        Comando = "D:\app\dev\rust\cargo\bin\cargo.exe test --workspace"
        LogFile = "D:\proj\mini\project-mini\logs\fase5-cargo-test.log"
    }
)
