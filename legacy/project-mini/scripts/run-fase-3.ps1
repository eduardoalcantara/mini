...
$comandos = @(
    @{
        Descricao = "FASE 3: Verificação de Sintaxe (cargo check)"
        Comando = "D:\app\dev\rust\cargo\bin\cargo.exe check --package mini --package mini_core"
        LogFile = "D:\proj\mini\project-mini\logs\fase3-cargo-check.log"
    },
    @{
        Descricao = "FASE 3: Testes Unitários (cargo test)"
        Comando = "D:\app\dev\rust\cargo\bin\cargo.exe test --package mini_core"
        LogFile = "D:\proj\mini\project-mini\logs\fase3-cargo-test.log"
    },
    @{
        Descricao = "FASE 3: Compilação Debug (cargo build --package mini --package mini_core)"
        Comando = "D:\app\dev\rust\cargo\bin\cargo.exe build --package mini --package mini_core"
        LogFile = "D:\proj\mini\project-mini\logs\fase3-cargo-build.log"
    }
)
...
for ($i = 0; $i -lt $comandos.Count; $i++) {
    ...
    try {
        ...
        if ($LASTEXITCODE -ne $null -and $LASTEXITCODE -ne 0) {
            $erroEncontrado = $true
            $tipoErro = "exit-code"
            Write-Host "✗ O comando retornou um erro" -ForegroundColor Red
            Write-Host "[Tipo de erro: $tipoErro]" -ForegroundColor DarkGray

            # IMPRIMIR LOG COMPLETO NA TELA (para o Cursor ver)
            if (Test-Path $logFile) {
                Write-Host ""
                Write-Host ("="*80) -ForegroundColor Red
                Write-Host "CONTEÚDO DO LOG DE ERRO:" -ForegroundColor Yellow
                Write-Host ("="*80) -ForegroundColor Red
    ...
    if (-not $erroEncontrado -and (Test-Path $logFile) -and $palavrasErro.Count -gt 0) {
        ...
        if ($linhasComErro) {
            ...
            if (Test-Path $logFile) {
                Write-Host ""
                Write-Host ("="*80) -ForegroundColor Red
                Write-Host "CONTEÚDO COMPLETO DO LOG:" -ForegroundColor Yellow
                Write-Host ("="*80) -ForegroundColor Red
                Get-Content $logFile | ForEach-Object { Write-Host $_ }
                Write-Host ("="*80) -ForegroundColor Red
                Write-Host ""
