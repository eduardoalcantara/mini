...
$comandos = @(
    @{
        Descricao = "FASE 7: Verificação de Sintaxe (cargo check)"
        Comando = "D:\app\dev\rust\cargo\bin\cargo.exe check --workspace"
        LogFile = "D:\proj\mini\project-mini\logs\fase7-cargo-check.log"
    },
    @{
        Descricao = "FASE 7: Testes Unitários (cargo test)"
        Comando = "D:\app\dev\rust\cargo\bin\cargo.exe test --workspace"
        LogFile = "D:\proj\mini\project-mini\logs\fase7-cargo-test.log"
    }
)
