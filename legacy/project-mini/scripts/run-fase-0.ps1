...
$comandos = @(
    @{
        Descricao = "FASE 0: Verificação de Sintaxe (cargo check)"
        Comando = "D:\app\dev\rust\cargo\bin\cargo.exe check --package mini"
        LogFile = "D:\proj\mini\project-mini\logs\fase0-cargo-check.log"
    },
    @{
        Descricao = "FASE 0: Compilação Debug (cargo build --package mini)"
        Comando = "D:\app\dev\rust\cargo\bin\cargo.exe build --package mini"
        LogFile = "D:\proj\mini\project-mini\logs\fase0-cargo-build.log"
    }
)
