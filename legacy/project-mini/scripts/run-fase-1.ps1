...
$comandos = @(
    @{
        Descricao = "FASE 1: Verificação de Sintaxe (cargo check)"
        Comando = "D:\app\dev\rust\cargo\bin\cargo.exe check --package mini --package mini_ui"
        LogFile = "D:\proj\mini\project-mini\logs\fase1-cargo-check.log"
    },
    @{
        Descricao = "FASE 1: Testes Unitários (cargo test)"
        Comando = "D:\app\dev\rust\cargo\bin\cargo.exe test --package mini_ui"
        LogFile = "D:\proj\mini\project-mini\logs\fase1-cargo-test.log"
    },
    @{
        Descricao = "FASE 1: Compilação Debug (cargo build --package mini --package mini_ui)"
        Comando = "D:\app\dev\rust\cargo\bin\cargo.exe build --package mini --package mini_ui"
        LogFile = "D:\proj\mini\project-mini\logs\fase1-cargo-build.log"
    }
)
