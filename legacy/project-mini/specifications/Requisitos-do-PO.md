...
### Seleção Retangular de Texto

- O MINI deve ter a mesma funcionalidade de seleção retangular do Notepad++ que permite usar Ctrl+Shit ou Ctrl+Alt com o botão do mouse pressionado para selecionar o texto com X,Y a partir de onde botão do mouse foi segurando, até o local W,Z onde o botão do mouse foi solto, criando um retângulo de texto selecionado perfeito, dentro do texto do arquivo. No momento o ZED tenta fazer parecido mas falha na última coluna de texto, tirando letras de algumas linhas ou colocando letras que estão de fora do retângulo selecionado.
...
- O Zed, que é a base do MINI já possui uma barra chamada `Project Search` que podemos usar como uma ferramenta não somente para encontrar arquivos no projeto (pasta aberta), mas também para encontrar qualquer arquivo no SO (Um Ctrl+P do VSCode, só que turbinado), quando o MINI não estiver em modo de pasta. Isso seria muito útil para o usuário, bem como uma opção para encontrar texto dentro dos arquivos.
...
- O Menu `Arquivo` também precisa ter um submenu com os arquivos já fechados recentemente para que o usuário possa abrí-los mais facilmente.
