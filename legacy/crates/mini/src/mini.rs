...
#[cfg(target_os = "windows")]
use mini_ui::tray_icon::TrayIcon;
#[cfg(target_os = "windows")]
use windows::Win32::Foundation::HWND;

// Global tray icon instance
#[cfg(target_os = "windows")]
static TRAY_ICON: OnceLock<Arc<Mutex<Option<TrayIcon>>>> = OnceLock::new();
...
pub fn initialize_workspace(
    ...
    cx.observe_new(move |workspace: &mut Workspace, window, cx| {
        ...
        // Initialize tray icon (only if enabled in settings)
        #[cfg(target_os = "windows")]
        {
            use settings::SettingsStore;
            use workspace::WorkspaceSettings;
            let settings_store = SettingsStore::global(cx);
            let workspace_settings = settings_store.get::<WorkspaceSettings>(None);
            let enable_tray = workspace_settings.enable_tray_icon;

            if enable_tray {
                if let Ok(tray_icon) = TrayIcon::new() {
                    // Get native window handle using HasWindowHandle trait
                    use raw_window_handle::{HasWindowHandle, RawWindowHandle};
                    if let Ok(raw_handle) = HasWindowHandle::window_handle(window) {
                        if let RawWindowHandle::Win32(win32_handle) = raw_handle.as_raw() {
                            // Convert raw_window_handle to windows crate HWND
                            // HWND in windows 0.61 is: HWND(*mut c_void)
                            // raw_window_handle provides: NonZero<*mut c_void>
                            use std::ffi::c_void;
                            let hwnd_ptr = win32_handle.hwnd.get() as *mut c_void;
                            if !hwnd_ptr.is_null() {
                                // HWND is a tuple struct: HWND(*mut c_void)
                                // Create HWND directly from the pointer
                                let hwnd = HWND(hwnd_ptr);
                                tray_icon.set_window_handle(hwnd);

                                // Show tray icon (but keep it hidden until window is minimized)
                                if let Err(e) = tray_icon.show() {
                                    log::warn!("Failed to show tray icon: {}", e);
                                }

                                // Install hook to intercept minimize button clicks
                                if let Err(e) = tray_icon.install_minimize_hook() {
                                    log::warn!("Failed to install minimize hook: {}", e);
                                }

                                // Store globally
                                TRAY_ICON.get_or_init(|| Arc::new(Mutex::new(Some(tray_icon))));
...
fn register_actions(
    ...
    workspace
        ...
        .register_action(|_workspace, _: &Minimize, window, cx| {
            #[cfg(target_os = "windows")]
            {
                use settings::SettingsStore;
                use workspace::WorkspaceSettings;
                ...
                // Hide window from taskbar and show tray icon (only if enabled)
                if enable_tray {
                    if let Some(tray_icon_arc) = TRAY_ICON.get() {
                        if let Some(tray_icon) = tray_icon_arc.lock().as_ref() {
                            log::info!("Minimize action: hiding window completely");
                            // Hide the window completely (this removes it from taskbar)
                            if let Err(e) = tray_icon.hide_window_from_taskbar() {
                                log::warn!("Failed to hide window: {}", e);
                            }
                            return; // Don't call minimize_window() - we're hiding it instead
                        }
                    }
                }
            }
